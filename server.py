...
from axon_api.services import connection_config_state as connection_config_state_service
...
async def agent_endpoint(body: AgentRequest, request: Request):
    """Compatibility facade for the extracted agent SSE route."""
    return await _agent_route_handlers.agent_endpoint(body, request)
...
def _resolved_api_runtime(settings: dict, provider_id: str, conn=None):
    return runtime_truth_service.resolved_api_runtime(
        settings,
        provider_id,
        provider_registry_module=provider_registry,
        devvault_module=devvault,
...
def _selected_api_runtime_truth(settings: dict, conn=None):
    return runtime_truth_service.selected_api_runtime_truth(
        settings,
        provider_registry_module=provider_registry,
...
def _is_chat_history_db_corruption(exc: Exception) -> bool:
    return chat_history_runtime_service.is_chat_history_db_corruption(
        exc,
        sqlite_database_error_cls=_sqlite3.DatabaseError,
    )


def _chat_history_db_detail() -> str:
    return chat_history_runtime_service.chat_history_db_detail()


async def _load_chat_history_rows(conn, project_id: Optional[int] = None, limit: int = 20, degrade_to_empty: bool = False):
    return await chat_history_runtime_service.load_chat_history_rows(
        conn,
        db_module=devdb,
        project_id=project_id,
        limit=limit,
        degrade_to_empty=degrade_to_empty,
        is_chat_history_db_corruption_fn=_is_chat_history_db_corruption,
        chat_history_db_detail_fn=_chat_history_db_detail,
        http_exception_cls=HTTPException,
    )


def _read_settings_sync() -> dict[str, str]:
    return settings_runtime_state_service.read_settings_sync(
        devdb.DB_PATH,
        managed_connection_fn=managed_connection,
        sqlite_row_factory=_sqlite3.Row,
    )


def _connection_config(settings: Optional[dict[str, str]] = None) -> dict:
    return connection_config_state_service.connection_config(
        settings,
        read_settings_sync_fn=_read_settings_sync,
        setting_truthy_fn=_setting_truthy,
    )


async def _runtime_truth_for_settings(settings: dict, conn=None, *, backend_override: str = ""):
    return await runtime_truth_service.runtime_truth_for_settings(
        settings,
        selected_api_runtime_truth_fn=_selected_api_runtime_truth,
        selected_cli_path_fn=composer_runtime.selected_cli_path,
        cli_runtime_family_fn=brain._cli_runtime_family,
        claude_cli_runtime_module=claude_cli_runtime,
        codex_cli_runtime_module=codex_cli_runtime,
        resolve_selected_cli_binary_fn=brain._resolve_selected_cli_binary,
        cli_runtime_key_fn=brain._cli_runtime_key,
        current_cli_cooldown_fn=current_cli_cooldown,
        build_runtime_truth_fn=runtime_truth_service.build_runtime_truth,
        ollama_service_status_fn=_ollama_service_status,
        conn=conn,
...
def _parse_stored_chat_message_base(raw_content: str) -> dict[str, object]:
...
_ops_runtime_facade = ops_runtime_facade_service.OpsRuntimeFacade(
    live_operator_snapshot=_live_operator_snapshot,
    browser_action_state=_browser_action_state,
    connection_config_fn=_connection_config,
    read_tunnel_url_fn=_read_tunnel_url,
    domain_probe_cache=_domain_probe_cache,
    time_module=_time,
    urlrequest_module=_urlrequest,
    urlerror_module=_urlerror,
    json_module=_json,
)

_set_live_operator = _ops_runtime_facade.set_live_operator
_attach_preview_browser = _ops_runtime_facade.attach_preview_browser
_connection_snapshot = _ops_runtime_facade.connection_snapshot
_auto_session_live_operator = _ops_runtime_facade.auto_session_live_operator
_task_sandbox_live_operator = _ops_runtime_facade.task_sandbox_live_operator
_now_iso = _ops_runtime_facade.now_iso

_chat_runtime_facade = chat_runtime_facade_service.ChatRuntimeFacade(
    brain_module=brain,
    db_module=devdb,
    devvault_module=devvault,
    provider_registry_module=provider_registry,
    ...
    resource_bank_module=resource_bank,
    console_command_service_module=console_command_service,
    select_history_for_chat_fn=select_history_for_chat,
    cli_session_persistence_enabled=cli_session_persistence_enabled,
    resolve_model_for_role=resolve_model_for_role,
    json_module=_json,
    memory_sync_cache=_memory_sync_cache,
    memory_sync_cache_ttl_seconds=_memory_sync_cache_ttl_seconds,
    ...
    set_live_operator_fn=_set_live_operator,
    persist_chat_reply_fn=_persist_chat_reply,
    stored_chat_message_fn=_stored_chat_message_base,
    parse_stored_chat_message_fn=_parse_stored_chat_message_base,
)

_effective_ai_params = _chat_runtime_facade.effective_ai_params
_maybe_handle_chat_console_command = _chat_runtime_facade.maybe_handle_chat_console_command
_stored_chat_message = _chat_runtime_facade.stored_chat_message
_stored_message_with_resources = _chat_runtime_facade.stored_message_with_resources
_parse_stored_chat_message = _chat_runtime_facade.parse_stored_chat_message
_history_messages_from_rows = _chat_runtime_facade.history_messages_from_rows
_serialize_chat_history_row = _chat_runtime_facade.serialize_chat_history_row
...
async def _ingest_resource_bytes(
    ...
    *,
    ...
    return await resource_route_state_service.ingest_resource_bytes(
        conn,
        title=title,
        filename=filename,
        content=content,
...
def _python_module_available(name: str) -> bool:
    return local_voice_dependencies_service.python_module_available(
        name,
...
def _resolve_ffmpeg_path() -> str:
    return local_voice_dependencies_service.resolve_ffmpeg_path(
...
def _piper_python_available() -> bool:
    return local_voice_dependencies_service.piper_python_available(
        python_module_available_fn=_python_module_available,
    )
...
async def _workspace_preview_target(project_id: int, auto_session_id: str = ""):
    return await workspace_sandbox_service.workspace_preview_target(
        project_id,
        auto_session_id,
        db_module=devdb,
...
async def _task_sandbox_ai_params(settings: dict, *, conn, runtime_override: dict[str, str] | None = None) -> dict:
    return await workspace_sandbox_service.task_sandbox_ai_params(
        settings,
        conn=conn,
        runtime_override=runtime_override,
        provider_registry_module=provider_registry,
        effective_ai_params_fn=_effective_ai_params,
    )


def _serialize_task_sandbox(meta: dict | None, *, include_report: bool = False) -> dict | None:
    return workspace_sandbox_service.serialize_task_sandbox(meta, include_report=include_report)


def _task_sandbox_runtime_override(payload) -> dict[str, str]:
    return workspace_sandbox_service.task_sandbox_runtime_override(
        payload,
        provider_registry_module=provider_registry,
    )
...
_task_sandbox_handlers = task_sandbox_routes.TaskSandboxHandlers(
    db_module=devdb,
    brain_module=brain,
    task_sandbox_service=task_sandbox_service,
    task_sandbox_runs=_task_sandbox_runs,
    now_iso=_now_iso,
    set_live_operator=_set_live_operator,
    serialize_task_sandbox=_serialize_task_sandbox,
    get_task_with_project=workspace_sandbox_service.get_task_with_project,
    task_sandbox_prompt=workspace_sandbox_service.task_sandbox_prompt,
    task_sandbox_ai_params=_task_sandbox_ai_params,
    task_sandbox_live_operator=_task_sandbox_live_operator,
    normalized_autonomy_profile=composer_runtime.normalized_autonomy_profile,
    normalized_runtime_permissions_mode=composer_runtime.normalized_runtime_permissions_mode,
    normalized_external_fetch_policy=composer_runtime.normalized_external_fetch_policy,
    task_sandbox_runtime_override=_task_sandbox_runtime_override,
...
_run_task_sandbox_background = _task_sandbox_handlers.run_task_sandbox_background
...
async def _queue_task_sandbox_run(
    ...
    *,
    ...
    return await _task_sandbox_handlers.queue_task_sandbox_run(
        task_id,
        resume=resume,
        runtime_override=runtime_override,
        run_task_sandbox_background=_run_task_sandbox_background,
    )
